        ...
        if (validatorState.nickname && validatorState.password) {
          ...
          if (dataFromDb.isLoginCorrect) {
            ...
            if (dataFromDb.message === 'Неверный пароль') {
              ...
              const authRemember = document.querySelector('#auth__remember');
              const authNickname = document.querySelector('#auth__nickname');
              const authPassword = document.querySelector('#auth__password');
              rememberButton.onclick = handleRemember;
              function handleRemember() {
                ...
                email.classList.remove('displayNone');
                authButton.textContent = 'Сброс пароля';
                authInputEmail.value = '';
                validationAll.textContent = '';
